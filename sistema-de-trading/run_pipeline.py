...
def main(args: argparse.Namespace) -> None:
    ...
    # ------------------------------------------------------------------
    # 8. Asignación simple Top-N long/short (laboratorio honesto sin solver)
    # ------------------------------------------------------------------
    import pandas as _pd  # alias local para evitar conflictos

    N_LONG = 5
    N_SHORT = 5
    LEVERAGE = config.apalancamiento_bruto_max  # apalancamiento bruto total

    print(
        f"Esquema Top-N long/short: N_LONG={N_LONG}, "
    ...
    if pred_col not in df_pred.columns:
        raise ValueError(
            f"La columna de predicciones '{pred_col}' no existe en df_pred. "
            f"Modelos disponibles: {config.modelos}"
        )

    # Construimos un DataFrame de señales
    sig = df_pred[["date", "ticker", pred_col]].copy()
    sig.rename(columns={pred_col: "expected_return"}, inplace=True)

    # Inicializamos columna de pesos objetivo
    ...
    df_weights = pd.concat(weights_rows, ignore_index=True)
    ...
    reporter.generate(
        output_path,
        metrics,
        {"predictions": df_pred, "weights": df_weights},
        df_equity,
        df_trades,
        fi_dict,
    )
